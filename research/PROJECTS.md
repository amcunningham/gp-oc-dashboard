...
|---|---------|-------------------|--------|-------------|------|
| 1 | **The tool** | Public explorer + practice page; GPs / practices | Live (69 visitors 7–12 Jul); 13 Jul quick-wins done, uncommitted | DONE 13 Jul: per-card provenance labels, per-answer sources, architecture trim, footer + `about.html` draft (T&Cs/privacy), data-flow claims corrected, list-size policy applied to pages, Worker log-salt (deployed). AMC registered with ICO (sole trader; registration reference ZC197414, expires 12 Jul 2027). Next: commit + push; finish about page (date, Z-number, drop DRAFT banner); interpretation-first summary card; MCP decision. Bug noted: name lookup matched wrong county for "dove valley" | `explore.html`, `mypractice.html`, `about.html`, `predictors.html`, `PRACTICE_TOOL_DESIGN.md` |
