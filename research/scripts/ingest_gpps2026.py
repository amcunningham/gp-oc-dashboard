...
evaluative base). National headlines weight practices by that base so each PATIENT counts
equally (the official convention) -- a plain mean across practices runs high on access items.
...
QSET = {
    ...
    "deflection_2026":          ("gpcontactnextstep_3.pct",        True),  # Q12 contact again
...
KEY_STEM = "ad_practicecode"
CRITICAL = ["overallexp.pcteval", "localgpservicesprefhpsee.pcteval",
            "gpcontactnextstep_3.pct", KEY_STEM]

# Headline items reported as a respondent-weighted England figure (official convention).
HEADLINE = {
...
# Per-practice survey-weighted evaluative base for headline metrics -> weight column in xsec.
# Weighting an area average (PCN/ICB/England) by these makes each PATIENT count equally,
# reproducing the official GPPS figure exactly (gpps_n is a decent proxy but not exact).
BASES = {
    "satisfaction_basew": "overallexp.baseevalw",
    "continuity_basew":   "localgpservicesprefhpsee.baseevalw",
    "access_basew":       "gpcontactoverall.baseevalw",
    "phone_basew":        "localgpservicesphone.baseevalw",
}
...
def integrity_check(present_cols):
    ...
    if missing:
        ...
        print("    Inspect the 2026 questionnaire changelog before trusting 2025->2026, then")
        print("    update QSET stems to match.")
    else:
...
def national_figure(con, stem):
    """Respondent-weighted England headline: weight each practice's positive % (.pcteval)
    by its survey-weighted evaluative base (.baseevalw), so each patient counts equally."""
    pe, be = f'"{stem}.pcteval"', f'"{stem}.baseevalw"'
    q = f"""
      SELECT SUM(v*w)/NULLIF(SUM(w),0)*100
...
def main():
    ...
    for out, (src_col, x100) in QSET.items():
        if src_col in present_cols:
            ...
            selects.append(f"CAST(NULL AS DOUBLE) AS {out}")
    for out, src_col in BASES.items():
        selects.append(f'{clean(src_col, False)} AS {out}' if src_col in present_cols
                       else f"CAST(NULL AS DOUBLE) AS {out}")
    ...
    # Respondent-weighted England headlines (the figures to quote publicly).
